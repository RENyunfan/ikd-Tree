...
struct ikdTree_PointType {
    float x, y, z;
    explicit ikdTree_PointType(const float & px = 0.0f,
        const float & py = 0.0f,
        const float & pz = 0.0f) : x(px), y(py), z(pz) {};
};
...
public:
    explicit MANUAL_Q(int max_capacity = Q_LEN) {
        q.resize(max_capacity);
    }
...
public:
    ...
    class MANUAL_HEAP {
    public:
        explicit MANUAL_HEAP(const int & max_capacity = 100) {
            cap = max_capacity;
...
public:
    explicit KD_TREE(const float & delete_param = 0.5, const float & balance_param = 0.6, const float & box_length = 0.2);
    ~KD_TREE();
    ...
    void Nearest_Search(PointType point, int k_nearest, PointVector& Nearest_Points, std::vector<float>& Point_Distance,
